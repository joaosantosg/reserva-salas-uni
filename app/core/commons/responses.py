...
class RespostaBase(BaseModel):
    ...
    status: str = Field(
        default="sucesso", description="Status da resposta (sucesso/erro)"
    )
    mensagem: Optional[str] = Field(
        default=None, description="Mensagem opcional da resposta"
    )
...
class ParametrosPaginacao(BaseModel):
    ...
    ordenacao: str = Field(
        default="desc", description="Direção da ordenação (asc/desc)"
    )
