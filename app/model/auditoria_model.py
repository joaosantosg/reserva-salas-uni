...
class AuditoriaReserva(BaseModel):
    ...
    dados_anteriores = Column(
        JSON, comment="Estado anterior da reserva em formato JSON"
    )
    dados_novos = Column(JSON, comment="Novo estado da reserva em formato JSON")
    usuario_id = Column(
        UUID(as_uuid=True),
    ...
    ip_address = Column(String(45), comment="Endereço IP do usuário (suporte a IPv6)")
    motivo = Column(
        String(255),
        nullable=True,
