...
class ReservaRecorrente(BaseModel):
    ...
    frequencia = Column(
        Enum(FrequenciaEnum), nullable=False, comment="Frequência da reserva recorrente"
    )
    dia_da_semana = Column(
        ARRAY(Integer), nullable=True, comment="Dia da semana (0=segunda, 6=domingo)"
    )  # Apenas para frequência semanal
    dia_do_mes = Column(
        Integer, nullable=True, comment="Dia do mês (1-31)"
    )  # Apenas para frequência mensal
    hora_inicio = Column(
        Time, nullable=False, comment="Hora de início da reserva recorrente"
    )
    hora_fim = Column(Time, nullable=False, comment="Hora de fim da reserva recorrente")
    data_inicio = Column(
        Date, nullable=False, comment="Data de início da reserva recorrente"
    )
