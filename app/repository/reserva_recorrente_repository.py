...
from app.schema.reserva_schema import ReservaRecorrenteFiltros, ReservasRecorrentesPaginadas
...
class ReservaRecorrenteRepository(BaseRepository):
    ...
    def get_by_query(self, query: ReservaRecorrenteFiltros) -> ReservasRecorrentesPaginadas:
    ...
    def check_conflict(self, sala_id: str, data_inicio: date, data_fim: date, 
                      hora_inicio: time, hora_fim: time, 
                      dia_da_semana: List[int], exclude_id: Optional[str] = None) -> bool:
    ...
    def get_by_period(self, sala_id: str, data_inicio: date, data_fim: date) -> List[ReservaRecorrente]:
        """Busca reservas recorrentes de uma sala em um período"""
        return self.session.query(ReservaRecorrente).filter(
            ReservaRecorrente.sala_id == sala_id,
            ReservaRecorrente.data_inicio <= data_fim,
            ReservaRecorrente.data_fim >= data_inicio
        ).all() 