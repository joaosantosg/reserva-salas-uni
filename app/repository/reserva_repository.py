...
class ReservaRepository(BaseRepository):
    ...
    def get_by_period(self, sala_id: str, inicio: datetime, fim: datetime) -> List[Reserva]:
        """Busca reservas de uma sala em um período"""
        return self.session.query(Reserva).filter(
            Reserva.sala_id == sala_id,
            Reserva.inicio >= inicio,
            Reserva.fim <= fim
        ).all()

    def soft_delete_reservas_recorrentes(self, reserva_recorrente_id: UUID, usuario_id: UUID) -> None:
