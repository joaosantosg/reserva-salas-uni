...
from app.schema.bloco_schema import BlocoCreate, BlocoUpdate, BlocoFiltros, BlocosPaginados
...
class BlocoService(BaseService):
    ...
    def create(self, bloco_data: BlocoCreate) -> Bloco:
        ...
        if self.bloco_repository.get_by_identificacao(bloco_data.identificacao):
            raise BusinessException(f"Já existe um bloco com a identificação {bloco_data.identificacao}")
