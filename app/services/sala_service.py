...
class SalaService(BaseService):
    ...
    def __init__(self, sala_repository: SalaRepository, bloco_repository: BlocoRepository):
    ...
    def create(self, sala_data: SalaCreate) -> Sala:
        ...
        if self.sala_repository.check_identificacao_sala_exists(sala_data.bloco_id, sala_data.identificacao_sala):
            raise BusinessException(f"Já existe uma sala com a identificação {sala_data.identificacao_sala} neste bloco")
        
        # Validação da capacidade máxima
        if sala_data.capacidade_maxima <= 0:
            raise BusinessException("A capacidade máxima da sala deve ser maior que zero")
        
        # Validação de curso restrito
        if sala_data.uso_restrito and not sala_data.curso_restrito:
            raise BusinessException("É necessário informar o curso restrito para salas de uso restrito")
    ...
    def update(self, sala_id: UUID, sala_data: SalaUpdate) -> Sala:
        ...
        # Verificar se já existe uma sala com a mesma identificação no bloco
        if sala_data.identificacao_sala and sala_data.identificacao_sala != sala_in_db.identificacao_sala:
            if self.sala_repository.check_identificacao_sala_exists(sala_in_db.bloco_id, sala_data.identificacao_sala, sala_id):
                raise BusinessException(f"Já existe uma sala com a identificação {sala_data.identificacao_sala} neste bloco")
        ...
        # Validação de curso restrito
        uso_restrito = sala_data.uso_restrito if sala_data.uso_restrito is not None else sala_in_db.uso_restrito
        curso_restrito = sala_data.curso_restrito if sala_data.curso_restrito is not None else sala_in_db.curso_restrito
